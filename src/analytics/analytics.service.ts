...
export class AnalyticsService {
  ...
  async generateImpressions(totalRecords: number): Promise<void> {
  const tableName = 'v5_Impressions_2025_01_11';
  ...
  for (let hour = 0; hour < 24; hour++) {
    for (let i = 0; i < recordsPerHour; i++) {
      ...
      data.push({
        ...
        ssp_endpoint_id: faker.number.int({ min: 1, max: 100 }),
        dsp_endpoint_id: faker.number.int({ min: 1, max: 100 }),
        dsp_spend: faker.number.float({ min: 0.01, max: 100}),
        ssp_spend: faker.number.float({ min: 0.01, max: 100}),
        price: faker.number.float({ min: 0.01, max: 10}),
        crid: faker.string.uuid(),
        cid: faker.string.uuid(),
        pmp_flag: faker.number.int({ min: 0, max: 1 }),
...
async generateNurl(totalRecords: number): Promise<void> {
  const tableName = 'v5_Nurls_2024_12_16';
